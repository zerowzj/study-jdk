...
public class FixedThreadPool3_Main {
    ...
    private class Task implements Callable<Result> {
        ...
        public Result call() throws Exception {
            log.info("task_no={}, sleep {}s", taskNo, value);
            Sleeps.seconds(value);
            Result result = new Result(taskNo, value * 1000);
            log.info("task_no={} finish", taskNo);
            return result;
        }
    }
    ...
    private void test() {
        ...
        for (Future<Result> future : futureLt) {
            try {
                ...
                log.info("task_no={}, begin_time={}, end_time={}", result.taskNo, result.getValue());
            } catch (InterruptedException | ExecutionException ex) {
                ex.printStackTrace();
            }
        }
        log.info("cost_time={} ms", System.currentTimeMillis() - start);
        pool.shutdown();
    }
